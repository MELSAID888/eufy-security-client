        ...
        try {
            ...
            switch (message.commandId) {
                ...
                case CommandType.CMD_GET_DEVICE_PING:
                    ...
                    break;
                case CommandType.CMD_P2P_PUSH_NOTIFICATION:
                    try {
                        const str = getNullTerminatedString(data, "utf8");
                        rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - CMD_P2P_PUSH_NOTIFICATION`, { stationSN: this.rawStation.station_sn, payload: str });
                        this.emit("push message", parseJSON(str, rootP2PLogger) as PushMessage);
                    } catch (err) {
                        const error = ensureError(err);
                        rootP2PLogger.error(`Handle DATA ${P2PDataType[message.dataType]} - CMD_P2P_PUSH_NOTIFICATION - Error`, {
